...
This workflow requires native function calling, a saved chat and a server-connected model. The administrator may set `generation_model_id`; an empty value resolves the current workspace model to its configured `base_model_id`. Workspace tools/skills and preset parameters are not applied again; available conversation context is retained. Explicit override IDs are also resolved to their base. Server-side provider/logging Pipes are supported and retained in the request path; they must handle non-streaming requests without tools or live chat event routing. Arena and browser-direct targets are not supported. Oversized context fails rather than silently dropping history. Tool success does not prove browser rendering succeeded.
