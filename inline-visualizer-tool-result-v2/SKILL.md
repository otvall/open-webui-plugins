---
name: visualize
description: Render interactive HTML/SVG from a JSON tool result in a saved Open WebUI chat. Call visualize(source_tool_call_id=...) only for explicit visualization requests after a Native tool call. The model writes the visual code and getToolData() supplies the JSON without regenerating its rows.
---
...
getToolData().then(function(result) {
  // Adapt this one line to the JSON shape returned by the source tool.
  const rows = Array.isArray(result) ? result : result.rows;
  if (!Array.isArray(rows)) throw new Error('Expected a rows array');
  new Chart(ctx, {
    type: 'bar',
    data: {
      labels: rows.map(row => row.period),
      datasets: [{ label: 'Revenue', data: rows.map(row => row.revenue),
