...
description: Renders interactive HTML/SVG visualizations inline in chat with optional data from a completed tool call. Requires "iframe Sandbox Allow Same Origin" to be enabled in Open WebUI Settings -> Interface. For design instructions, the model should call view_skill("visualize").
"""
...
class Tools:
    ...
    async def visualize(
        ...
        **If the user did not explicitly ask for a visual artifact, do not call visualize().**
        ...
        - When tool_call_id is supplied, copy the exact ID exposed on a completed tool call in this conversation. Treat it as opaque: do not construct it from the tool name or call count.
        ...
        :param tool_call_id: Exact tool_call_id/call_id copied from a completed tool call in the current conversation. The provider chooses its format; do not synthesize it. The selected result becomes available through getToolData().
        ...
        if tool_call_id is not None:
            if not isinstance(tool_call_id, str) or not tool_call_id.strip():
                return {
                    "status": "error",
                    "error": "Invalid tool_call_id",
                    "tool_call_id": tool_call_id,
                    "message": "Pass the exact ID of a completed earlier tool call.",
                }
            ...
            if not has_tool_data:
                return {
                    ...
                    "tool_call_id": tool_call_id,
                    "message": "No completed tool result with this ID is available. Run the data-producing tool first, then call visualize in a later tool round.",
                }
...
"""},
        ...
        tool_data_context = (
            "The selected tool result is available inside the iframe only via "
            "getToolData(). Do not reproduce it in the generated HTML or JavaScript. "
            if has_tool_data
            else ""
        )
        result_context = (
